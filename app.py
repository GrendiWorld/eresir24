...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'your-secret-key-change-in-production-' + hashlib.sha256(str(datetime.utcnow()).encode()).hexdigest()[:32])
...
def login():
    ...
    if request.method == 'POST':
        ...
        if client_ip not in brute_force_attempts:
            brute_force_attempts[client_ip] = {'count': 0, 'last_attempt': datetime.utcnow()}
        
        # Reset counter if last attempt was more than 15 minutes ago
        if (datetime.utcnow() - brute_force_attempts[client_ip]['last_attempt']).total_seconds() > 900:
        ...
        if admin and check_password_hash(admin.password_hash, password):
            ...
            brute_force_attempts[client_ip]['last_attempt'] = datetime.utcnow()
...
@rate_limit(seconds=10, max_requests=20)  # 20 requests per minute max
@limiter.limit("30 per hour")  # Additional global limit
...
@rate_limit(seconds=10, max_requests=30)  # 30 requests per minute max
@limiter.limit("100 per hour")  # Additional global limit
...
@rate_limit(seconds=10, max_requests=50)  # 50 requests per minute max
@limiter.limit("200 per hour")  # Additional global limit
