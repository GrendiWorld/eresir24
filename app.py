...
class ActiveMedia(db.Model):
    ...
    week_start_date = db.Column(db.DateTime, default=get_utc_now, nullable=False)  # Week start date
    start_date = db.Column(db.DateTime, default=get_utc_now, index=True)
...
def active_media():
    ...
    for media in media_list:
        # Ensure week_start_date is timezone-aware (fix for old records)
        if media.week_start_date.tzinfo is None:
            media.week_start_date = media.week_start_date.replace(tzinfo=timezone.utc)
            db.session.commit()
